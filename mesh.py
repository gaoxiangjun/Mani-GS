...
class Mesh:
    ...
    def auto_normal(self):
        ...
        self.face_normals = safe_normalize(face_normals)
    ...
    def laplacian(self):
        with torch.no_grad():
            L = self._laplacian_uniform().double()
        loss = L.mm(self.v.double())
        loss = loss.norm(dim=1)
        loss = loss.mean()
        return loss
    
    def normal_consistency(self):
        edge_nrm = self.vn[self.edges]
        nc = (
            1.0 - torch.cosine_similarity(edge_nrm[:, 0], edge_nrm[:, 1], dim=-1)
        ).mean()
        return nc
    
    def normal_consistency_remove_sharp_edge(self):
        edge_nrm = self.vn[self.edges]
        nc = (
            1.0 - torch.cosine_similarity(edge_nrm[:, 0], edge_nrm[:, 1], dim=-1)
        )
        mask = nc > 0.1
        sharp_nc = nc[mask].mean()
        return sharp_nc
    
    @property
    def edges(self):
        if self._edges is None:
            self._edges = self._compute_edges().long()
        return self._edges
